...
class BertTrainer(object):
    ...
    def train(self):
        ...
        train_sampler = Subset(train_data, self.args.labeled)
        train_dataloader = DataLoader(train_sampler, batch_size=self.args.batch_size)

        for epoch in trange(int(self.args.epochs), desc="Epoch"):
            self.train_epoch(train_dataloader)
            dev_evaluator = BertEvaluator(self.model, self.processor, self.tokenizer, self.args, split='dev')
            _, _, _, dev_acc, dev_precision, dev_recall, dev_f1, dev_loss = dev_evaluator.get_scores()[0]
