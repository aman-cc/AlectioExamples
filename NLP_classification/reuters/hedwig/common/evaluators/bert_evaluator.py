...
class BertEvaluator(object):
    ...
    def get_scores(self, silent=False):
        ...
        if self.split in ['test']:#, 'dev']:
            eval_sampler = eval_data
        elif self.split == 'dev':
            eval_sampler = Subset(eval_data, [i for i in range(200)])
        else:
            eval_sampler = Subset(eval_data, self.args.labeled)
        eval_dataloader = DataLoader(eval_sampler, batch_size=self.args.batch_size)

        self.model.eval()

        total_loss = 0
        nb_eval_steps, nb_eval_examples = 0, 0
        predicted_labels, target_labels, logit_list = list(), list(), list()
        ...
        return [target_labels, predicted_labels, logit_list, accuracy, precision, recall, f1, avg_loss], ['target_labels', 'predicted_labels', 'pre_softmax', 'accuracy', 'precision', 'recall', 'f1', 'avg_loss']
