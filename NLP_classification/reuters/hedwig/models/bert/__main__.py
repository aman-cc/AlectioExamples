...
def evaluate_split(model, processor, tokenizer, args, split='dev'):
    ...
    pred, target, pre_softmax, accuracy, precision, recall, f1, avg_loss = evaluator.get_scores(silent=True)[0]
    print('\n' + LOG_HEADER)
    print(LOG_TEMPLATE.format(split.upper(), accuracy, precision, recall, f1, avg_loss))
    return pred, target, pre_softmax
...
def main(args):
    ...
    pred, target, pre_softmax = None, None, None
    ...
    if not args.trained_model:
        ...
        torch.save(model, os.path.join(args.snapshot_path))
        ...
        pred, target, pre_softmax = evaluate_split(model, processor, tokenizer, args, split=args.split)
        # pred, target, pre_softmax = pred.tolist(), target.tolist(), pre_softmax.tolist()
        # pred = pred[:, 0]
        # target = target[:, 0]
